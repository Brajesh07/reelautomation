...
## 8. `src/components/ReelCanvas.jsx`
- **Purpose**: The original legacy video rendering engine. 
- **Mechanics**:
  - Unlike the modern `renderSign.js` which captures off-screen and writes to FFmpeg WASM, `ReelCanvas.jsx` relied on the browser's `MediaRecorder` API to capture a live `<canvas>` stream (`canvas.captureStream()`).
  - It loops through all zodiacs inside a single massive GSAP timeline.
  - **Flaws / Why it was replaced**: `MediaRecorder` relies on real-time playback speed. If the browser lags, the resulting `.webm` video stutters or drops frames. It also couldn't produce raw MP4s without a secondary conversion step, prompting the architectural shift to the deterministic FFmpeg WASM pipeline.
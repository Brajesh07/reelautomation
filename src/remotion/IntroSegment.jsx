...
export const IntroSegment = ({ zodiacs = [] }) => {
  ...
  const zodiacNamesStr = zodiacs.map(z => z.name.toUpperCase()).join(', ');
  
  let revealedZodiacs = "";
  if (frame >= 150) {
    revealedZodiacs = zodiacNamesStr;
  } else if (frame >= 120) {
    revealedZodiacs = zodiacNamesStr.slice(0, Math.floor(zodiacTypingProgress * zodiacNamesStr.length));
  }
    ...
    <AbsoluteFill style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5, justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%', pointerEvents: 'none' }}>
          ...
          <p style={{ ...commonTextStyle, fontSize: '45px', maxWidth: '800px' }}>{revealedZodiacs}</p>

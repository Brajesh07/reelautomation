...
const FramePreview = () => {
  ...
  useEffect(() => {
    ...
    const totalImages =
      decorativeSources.length +
      uniqueZodiacNames.length +
      zodiacIconsArray.length;
  ...
  useEffect(() => {
      ...
      const addType = (k, txt, d) => {
        tl.to(
          counters,
          {
            ...
            ease: "none",
            onUpdate: () => {
              introState[`text${k.charAt(1)}`] = txt.substring(
                0,
    ...
    // ----------------------------------------------------------------
      ...
      zodiacs.forEach((zodiac, index) => {
        ...
        const icon =
          zodiacIconsRef.current[zodiac.name] || zodiacIconsRef.current["Leo"];
  ...
  return (
    <div className="relative flex items-start py-20 gap-3 md:flex-row flex-col">
      ...
      <div
        className="border-none overflow-hidden bg-black"
        style={{
          width: "405px",
          height: "720px",
      ...
      {!imagesLoaded && (
        <div className="text-white mt-2.5">Loading Assets...</div>
      )}
    </div>
  );
};

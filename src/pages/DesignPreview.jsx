...
const ringIconSources = [aries, taurus, gemini, cancer, leo, virgo, libra, scorpio, sagittarius, capricorn, aquarius, pisces]
...
const FONT_CONTROLS = {
    ...
    zodiac: [
        ...
        { key: 'sectionLabel', label: 'Section Labels', min: 20, max: 60, default: 40 },
        { key: 'boxContent', label: 'Box Content', min: 16, max: 52, default: 32 }
    ],
    outro: [
        { key: 'sectionLabel', label: 'Question Text', min: 20, max: 60, default: 42 },
        { key: 'boxContent', label: 'Box Content', min: 16, max: 52, default: 32 }
    ]
}
...
const ToolPanel = ({ selectedFrame, fontSizes, onChange }) => {
    ...
    if (!selectedFrame || controls.length === 0) return (
        <div className='w-64 shrink-0 bg-[#111] rounded-xl border border-white/10 p-5 flex items-center justify-center'>
            <p className='text-gray-600 text-sm text-center'>Select a frame to see<br />font controls</p>
        </div>
    )

    return (
        <div className='w-64 shrink-0 bg-[#111] rounded-xl border border-white/10 p-5 flex flex-col gap-5'>
            <h3 className='text-[#DAC477] font-bold text-sm uppercase tracking-widest'>Font Sizes</h3>

            {controls.map(ctrl => (
                <div key={ctrl.key} className='flex flex-col gap-2'>
                    <div className='flex justify-between items-center'>
                        <label className='text-gray-300 text-xs font-medium'>{ctrl.label}</label>
                    ...
                    <div className='flex justify-between text-gray-600 text-[10px]'>
                        <span>{ctrl.min}px</span><span>{ctrl.max}px</span>
                    </div>
                </div>
            ))}
            ...
            <button
                onClick={() => {
                    const resets = {}
                    controls.forEach(c => { resets[c.key] = c.default })
...
const DesignPreview = () => {
    ...
    const [fontSizes, setFontSizes] = useState(() => {
        try {
            ...
            return saved ? { ...DEFAULT_FONT_SIZES, ...JSON.parse(saved) } : { ...DEFAULT_FONT_SIZES }
        } catch { return { ...DEFAULT_FONT_SIZES } }
    })
    ...
    useEffect(() => {
        ...
        // Persist for recording (ReelCanvas reads this before starting)
    ...
    useEffect(() => {
        ...
        if (loaded?.zodiacs?.length) {
            ...
            fetch('/data.json')
                .then(r => r.json())
                .then(d => { if (d.zodiacs?.length) setZodiacs(d.zodiacs) })
    ...
    useEffect(() => {
        ...
        const total = decorativeSources.length + uniqueNames.length + ringIconSources.length
        let loaded = 0
        ...
        decorativeSources.forEach((src, i) => {
            const img = new Image()
            img.src = src
            img.onload = () => { loadedDeco[i] = img; imagesRef.current = loadedDeco; onLoad() }
        })
        ...
        uniqueNames.forEach(name => {
            ...
            img.onload = () => { zodiacIconsRef.current[name] = img; onLoad() }
        })
        ...
        ringIconSources.forEach((src, i) => {
            const img = new Image()
            img.src = src
            img.onload = () => { loadedRing[i] = img; ringImagesRef.current = loadedRing; onLoad() }
        })
    }, [zodiacs])
    ...
    useEffect(() => {
        ...
        if (selectedFrame === 'intro') {
            const state = {
                scale: 0, rotation: -60, opacity: 0, textFade: 1, showText: false,
            ...
            const update = () => {
                const textData = state.showText ? {
                    ...
                    line1: state.text1, line2: state.text2, line3: state.text3,
                    line4: state.text4, line5: state.text5
                } : null
                renderIntroFrame(ctx, {
                    scale: state.scale, rotation: state.rotation, opacity: state.opacity,
                    images: ringImagesRef.current, highlightedNames, textData
            ...
            tl.to(state, { scale: 1, opacity: 1, rotation: 120, duration: 3.5, ease: 'power2.out', onUpdate: update })
            ...
            const addType = (k, txt, at) => {
                tl.to(counters, {
                    ...
                    onUpdate: () => {
                        state[`text${k.charAt(1)}`] = txt.substring(0, Math.ceil(counters[k]))
                        update()
                    }
                }, at)
            }
            ...
            const animState = {
                ...
                zodiacName: '', showName: false, vibeText: '', showVibe: false,
                sections: [
            ...
            const update = () => {
                ...
                const sectionsData = animState.sections.map(s => ({
                    ...
                    anim: { labelOpacity: s.opacity, labelYOffset: s.yOffset, maskProgress: s.mask, showLabel: s.showLabel, showContent: s.showContent }
                }))
                renderDraftFrame(ctx, {
                    decorativeAnim: { opacity: animState.decoOpacity, yOffset: animState.decoY, images: imagesRef.current },
                    zodiacAnim: { icon: currentIcon, xOffset: animState.zodiacX, rotation: animState.zodiacRotation, opacity: animState.zodiacOpacity, name: animState.zodiacName, showName: animState.showName },
            ...
            if (currentIndex === 0) {
                tl.to(animState, { decoY: 0, decoOpacity: 0.4, duration: 1.5, ease: 'power2.out', onUpdate: update })
            } else {
                animState.decoY = 0; animState.decoOpacity = 0.4
            }
            tl.to(animState, { zodiacX: 0, zodiacRotation: 0, zodiacOpacity: 1, duration: 1.5, ease: 'power2.out', onUpdate: update }, '+=0')
            ...
            tl.to(nameCounter, {
                v: nameFull.length, duration: nameFull.length * 0.1, ease: 'none',
                onUpdate: () => { animState.zodiacName = nameFull.substring(0, Math.ceil(nameCounter.v)); update() }
            })
            ...
            tl.to(vibeCounter, {
                v: vibeFull.length, duration: vibeFull.length * 0.05, ease: 'none',
                onUpdate: () => { animState.vibeText = vibeFull.substring(0, Math.ceil(vibeCounter.v)); update() }
            })
            ...
            animState.sections.forEach((section, idx) => {
                ...
                tl.to(section, { opacity: 1, yOffset: 0, duration: 0.8, ease: 'power2.out', onUpdate: update })
                tl.set(section, { showContent: true }, '-=0.4')
                tl.to(section, { mask: 1, duration: 1.2, ease: 'power1.inOut', onUpdate: update })
            })
            ...
            if (!isLast) {
                ...
                animState.sections.forEach(s => tl.to(s, { opacity: 0, mask: 0, duration: 0.8 }, 'exit'))
                tl.to(animState, { showVibe: false, duration: 0.5 }, 'exit+=0.5')
                tl.call(() => setCurrentIndex(prev => prev + 1))
            } else {
                tl.to(animState, { decoOpacity: 0, zodiacOpacity: 0, duration: 2, ease: 'power2.inOut' }, 'finalExit')
                animState.sections.forEach(s => tl.to(s, { opacity: 0, mask: 0, duration: 1 }, 'finalExit'))
                tl.to(animState, { showVibe: false, showName: false, duration: 1 }, 'finalExit')
            }

            // ── OUTRO ──────────────────────────────────────────────────────────────
        } else if (selectedFrame === 'outro') {
            const state = { opacity: 0, rotation: -60, scale: 0, text1: '', boxWidth: 0 }

            const update = () => {
                renderOutroFrame(ctx, {
                    opacity: state.opacity, rotation: state.rotation, scale: state.scale,
                    text1: state.text1, boxWidth: state.boxWidth,
            ...
            tl.to(textCounter, {
                v: fullText.length, duration: fullText.length * 0.05, ease: 'none',
                onUpdate: () => { state.text1 = fullText.substring(0, Math.ceil(textCounter.v)); update() }
            })

            tl.to(state, { boxWidth: 100, duration: 1.5, ease: 'power2.out', onUpdate: update })
            tl.to({}, { duration: 2 })
            tl.to(state, { opacity: 0, duration: 1.5, ease: 'power2.inOut', onUpdate: update })
        }
    ...
    return (
        <div className='min-h-screen bg-[#1a1a1a] text-white flex flex-col'>
            ...
            <div className='flex items-center justify-between px-8 py-4 border-b border-white/10'>
                ...
                <Link to='/' className='text-sm text-[#DAC477] hover:underline'>← Back to Upload</Link>
            </div>
            ...
            <div className='flex-1 flex gap-6 px-8 py-6 justify-center items-start'>
                ...
                <div className='w-56 shrink-0 flex flex-col gap-4 sticky top-24'>
                    <div className='bg-[#111] rounded-xl border border-white/10 p-5 flex flex-col gap-3'>
                        <h3 className='text-[#DAC477] font-bold text-sm uppercase tracking-widest'>Frame</h3>
                        {[
                            ...
                            <button
                                ...
                                className={`w-full py-2.5 px-4 rounded-lg text-sm font-medium text-left transition-all duration-200 ${selectedFrame === f.val
                    ...
                    {selectedFrame === 'zodiac' && zodiacs.length > 0 && (
                        <div className='bg-[#111] rounded-xl border border-white/10 p-4 text-xs text-gray-400'>
                            <div className='font-semibold text-gray-300 mb-2'>Zodiac Cards</div>
                            {zodiacs.map((z, i) => (
                                <div key={i} className={`py-1 px-2 rounded ${i === currentIndex ? 'text-[#DAC477]' : ''}`}>
                                    {i === currentIndex ? '▶ ' : '○ '}{z.name}
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className='flex flex-col items-center gap-3'>
                    <div
                        ...
                        style={{ width: '405px', height: '720px', boxShadow: '0 0 40px rgba(0,0,0,0.6)' }}
                    >
                        <canvas
                            ref={canvasRef}
                            className='block'
                        ...
                        {!selectedFrame && (
                            <div className='absolute inset-0 flex items-center justify-center'>
                                <p className='text-gray-600 text-sm'>Select a frame to preview</p>
                            </div>
                        )}
                        {!imagesLoaded && (
                    ...
                    <p className='text-gray-600 text-xs'>1080 × 1920 canvas · previewed at 405 × 720</p>
                </div>
